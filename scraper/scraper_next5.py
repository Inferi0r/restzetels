...
def get_municipalities_slice(start_index_1based: int, end_index_1based: int) -> list[dict]:
    """Return municipalities slice using 1-based inclusive indices.
    Example: 5..10 returns 6 items (indices 4:10 in zero-based).
    """
    data = load_json(find_data_file("municipalities.json"))
    items = data.get("items", [])
    a = max(0, start_index_1based - 1)
    b = max(a, end_index_1based)
    return items[a:b]
...
def cmd_first10(args: argparse.Namespace) -> None:
    ensure_data_dir()
    # Load or create verified URLs
    verified_path = find_data_file("municipality_links_verified.json")
    if not os.path.exists(verified_path):
        data = load_json(find_data_file("municipalities.json"))
        verify_municipality_links(data.get("items", []))

    # Load stemlokalen
    default_stemlokalen = os.path.join(SCRIPT_DIR, "alle_stemlokalen.json")
    path_stemlokalen = args.stemlokalen or default_stemlokalen
    if not os.path.isabs(path_stemlokalen):
        # if given relative, try relative to script dir first
        cand = os.path.join(SCRIPT_DIR, path_stemlokalen)
        if os.path.exists(cand):
            path_stemlokalen = cand
    stemlokalen = load_stemlokalen(path_stemlokalen)

    # Determine first 10 municipalities
    first10 = get_first_n_municipalities(10)
    names = [m["name"] for m in first10]
    if getattr(args, "only", None):
        names = [n for n in names if n in set(args.only)]
    print(f"[first10] Target municipalities: {', '.join(names)}")

    # Clean existing dirs for the first10 (remove incorrect files)
    for n in names:
        d = os.path.join(SCRIPT_DIR, sanitize_filename(n))
        if os.path.isdir(d):
            print(f"[first10] Removing existing directory: {d}")
            shutil.rmtree(d)
    ...
    for n in names:
        ...
        if not start_url:
            print(f"[first10] Geen geldige URL voor {n}, overslaan")
        ...
        expected = len(sbs)
        print(f"[first10] {n}: {expected} stembureaus, crawling {start_url}")
        ...
        muni_dir = os.path.join(SCRIPT_DIR, sanitize_filename(n))
        ...
        for pdf in pdfs:
            ...
            try:
                ...
                if "application/pdf" not in r.headers.get("Content-Type", "").lower() and not urlparse(pdf["url"]).path.lower().endswith(".pdf"):
                    print(f"[first10] Skip non-PDF: {pdf['url']}")
                    continue
                ...
                if sid:
                    matched_ids.add(sid)
                found += 1
                print(f"[first10] Saved: {final_path}")
            except Exception as e:
                print(f"[first10] Error downloading {pdf['url']}: {e}")
    ...
    with open(os.path.join(DATA_DIR, "first10_summary.json"), "w", encoding="utf-8") as f:
        json.dump(summary, f, ensure_ascii=False, indent=2)
    print(f"[first10] Summary saved -> {os.path.join(DATA_DIR, 'first10_summary.json')}")


def cmd_next5(args: argparse.Namespace) -> None:
    ensure_data_dir()
    ...
    # Determine municipalities 5..10 (1-based, inclusive)
    slice_items = get_municipalities_slice(5, 10)
    names = [m["name"] for m in slice_items]
    if getattr(args, "only", None):
        names = [n for n in names if n in set(args.only)]
    print(f"[next5] Target municipalities: {', '.join(names)}")

    # Clean existing dirs for this slice
    for n in names:
        d = os.path.join(SCRIPT_DIR, sanitize_filename(n))
        if os.path.isdir(d):
            print(f"[next5] Removing existing directory: {d}")
            shutil.rmtree(d)
        os.makedirs(d, exist_ok=True)
    ...
    for n in names:
        start_url = get_verified_url_for_municipality(n)
        if not start_url:
            print(f"[next5] Geen geldige URL voor {n}, overslaan")
            summary[n] = {"start_url": None, "found": 0, "expected": 0, "missing": []}
            continue
        ...
        sbs = filter_stemlokalen_for_muni(stemlokalen, n)
        expected = len(sbs)
        print(f"[next5] {n}: {expected} stembureaus, crawling {start_url}")
        use_sm = n in ("Aalten", "Achtkarspelen") or args.use_sitemap
        seeds_map = load_extra_seeds()
        extra = seeds_map.get(n, [])
        pdfs = find_pdfs_for_municipality(
        ...
        for pdf in pdfs:
            ...
            try:
                ...
                r.raise_for_status()
                if "application/pdf" not in r.headers.get("Content-Type", "").lower() and not urlparse(pdf["url"]).path.lower().endswith(".pdf"):
                    print(f"[next5] Skip non-PDF: {pdf['url']}")
                    continue
                cd = r.headers.get("Content-Disposition", "")
                fname_from_cd = None
                if cd:
                ...
                if sid:
                    matched_ids.add(sid)
                found += 1
                print(f"[next5] Saved: {final_path}")
            except Exception as e:
                print(f"[next5] Error downloading {pdf['url']}: {e}")
    ...
    with open(os.path.join(DATA_DIR, "next5_summary.json"), "w", encoding="utf-8") as f:
        json.dump(summary, f, ensure_ascii=False, indent=2)
    print(f"[next5] Summary saved -> {os.path.join(DATA_DIR, 'next5_summary.json')}")
...
def download_pdfs(
    pdf_index: list[dict] | list,
    base_dir: str = os.getcwd(),
    delay: float = 0.1,
    limit_per_municipality: int | None = None,
) -> None:
    # Allow both complete index format and a simple list of one entry
    if isinstance(pdf_index, dict) and "results" in pdf_index:
        entries = pdf_index.get("results", [])
    else:
        entries = pdf_index
...
def build_argparser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="Scraper (first10 variant) voor processen-verbaal per stembureau (gemeenten)")
    ...
    s5 = sub.add_parser("first10", help="Voer end-to-end uit voor eerste 10 gemeenten m.b.v. alle_stemlokalen.json")
    s5.add_argument("--stemlokalen", type=str, default=os.path.join(SCRIPT_DIR, "alle_stemlokalen.json"), help="Pad naar alle_stemlokalen.json")
    ...
    s5.add_argument("--use-sitemap", action="store_true", help="Forceer sitemap discovery voor alle 10 gemeenten")
    s5.add_argument("--only", nargs='*', help="Alleen deze gemeenten uitvoeren (subset van eerste 10)")
    s5.set_defaults(func=cmd_first10)

    s7 = sub.add_parser("next5", help="Download gemeenten 5 t/m 10 alfabetisch m.b.v. alle_stemlokalen.json")
    s7.add_argument("--stemlokalen", type=str, default=os.path.join(SCRIPT_DIR, "alle_stemlokalen.json"), help="Pad naar alle_stemlokalen.json")
    s7.add_argument("--max-pages", type=int, default=40, help="Max pagina's per gemeente crawlen")
    s7.add_argument("--max-depth", type=int, default=3, help="Crawl diepte binnen zelfde site")
    s7.add_argument("--use-sitemap", action="store_true", help="Forceer sitemap discovery voor deze 5 gemeenten")
    s7.add_argument("--only", nargs='*', help="Alleen subset van gemeenten 5-10")
    s7.set_defaults(func=cmd_next5)

    # Convenience (kept): fetch a single municipality and download immediately (v2 parity)
