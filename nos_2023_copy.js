function loadDataFor2023() {
    const baseUrl = 'https://faas-ams3-2a2df116.doserverless.co/api/v1/web/fn-99532869-f9f1-44c3-ba3b-9af9d74b05e5/default/getdata';
...
async function fetchKiesgerechtigdenTotal(gemeentes, baseUrl) {
    let totalKiesgerechtigden = 0;

    try {
        const kiesgerechtigdenPromises = gemeentes.map(gemeente => {
            const gemeenteUrl = `${baseUrl}?year=2023&source=gemeente&cbs_code=${gemeente.gemeente.cbs_code}`;
            return fetch(gemeenteUrl).then(response => response.json());
        });

        const gemeenteData = await Promise.all(kiesgerechtigdenPromises);

        gemeenteData.forEach(data => {
            if (data.huidige_verkiezing && data.huidige_verkiezing.verkiezing_code === 'TK23') {
                totalKiesgerechtigden += data.huidige_verkiezing.kiesgerechtigden;
            }
        });

        // Update the content of the totalKiesgerechtigden element on the webpage
        document.getElementById('totalKiesgerechtigden').textContent = 
            'Kiesgerechtigden: ' + totalKiesgerechtigden.toLocaleString('nl-NL');
    } catch (error) {
        console.error('Error fetching data:', error);
        document.getElementById('totalKiesgerechtigden').textContent = 
            'Error fetching total Kiesgerechtigden';
    }
}
...
async function fetchData(baseUrl) {
    try {
        const indexUrl = `${baseUrl}?year=2023&source=nos`;
        ...
        // Call the function to count kiesgerechtigden with fetched gemeentes data
        await fetchKiesgerechtigdenTotal(data.gemeentes, baseUrl);
    } catch (error) {
